...
### A3 执行结果（2026-09-05 完成）

```
备份    511825  COMPLETED  108 GB / 43 文件，来源+完整性校验通过
重编译  511836  COMPLETED  1:22   exe cb75cc200c -> 0485ff9415
重跑    511837  COMPLETED  42:14  20 节点
        2020-01-01_01:00 -> 2024-01-01_00:00，35040 步 = 4x365x24
        SIGSEGV / ENDRUN / balance error 全 0
        收尾 nstep = 1524240，与原生产作业终点相同 -> 相位精确复现，无漂移
比较    512084  新旧 2024 restart 逐变量分析
```

**驱动索引轨迹核对**（解释为何只应差末端）：

```
旧(连续)  1908 起 46720 -> 1936 绕回 1 -> 1980 再绕回 1
          -> 1981-01-01 时 2920 -> 43 年后 128480，末条 tindex(2) 越界
新(A3)    2020 初始化 mod(mod(170,44)+2,44)=40 -> 116800 -> 4 年后 128480
两者 2020-01-01 均为 116800，轨迹相同
```
...
| 文件 | 结果 |
|---|---|
| `cpl.r` / `elm.rh0` / `elm.rh1` | 逐比特一致 |
| `elm.r` | 464 变量中 220 个有差异；结构（变量集/dtype/维度/形状）完全一致 |

| 类别 | 中位相对差 | 最大相对差 |
|---|---|---|
| `totsomc` | 1.2e-08 | 6.1e-07 |
| `soil4c_vr` | 1.4e-10 | 9.6e-07 |
| `soil1c_vr` | 7.8e-07 | 1.35e-03 |
| `deadstemc` | 4.2e-07 | 2.5e-05 |
| `leafc` | 7.8e-06 | 7.7e-05 |
| `T_SOISNO` | 0.00219 K（绝对） | 99.9 分位 2.91 K |

**极端值已定位，非物理量**：`T_SOISNO` 最大差 273.15 K 只出现在 **40 个元素**，
全部落在第 2/3/4 层（ELM 前 5 层是雪层）。根因是 `SNLSNO` 在
**18 / 1,226,904 列（0.0015%）**上雪层数差 1，导致失活/新激活槽位的值不同。

**结论**：差异与"仅末端约 3 小时驱动改变"完全相符，未发现无法由该机制解释
的变化。慢变碳库 1e-7~1e-10 的相对差远低于任何模式结构不确定性。新的 2024
存档接受为 future 情景的初始场。

### 未做的对照，及其理由

原可用保存的旧 exe（`e3sm.exe.pre_A3`，md5 `cb75cc200c…`）从同一 2020
checkpoint 重跑一次，以区分"差异来自修复"还是"来自中途重启"。**决定不做**：

- 该对照只影响**归因**，不影响**用哪份**——新存档是用不越界的代码产出的，
  旧那份在 2023 年末读过界外内存，无论归因如何都该用新的
- 差异量级（碳库 1e-7~1e-10）远低于任何有意义的阈值

**但由此留下一条未验证的性质，需要在用到它之前单独测**：

> **4km 上的重启无损性从未直接验证。** 测试 2a 是 0.5°、1 年、同一份代码。
> 当前不受影响（future 用 `finidat` startup 加载初始场，不是 `CONTINUE_RUN`），
> 但 **7 个 future run 是 2024-2100 的 77 年积分，很可能分段重启**——那时才
> 真正用到这个性质。届时应直接测一次（连续 vs 中途重启，同一份代码，
> 比对最终 restart），不要从本次 A3 的比较里推断。
