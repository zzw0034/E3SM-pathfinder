...
| 对象 | 逐比特一致 | 已记录豁免 | 未解释 | 判定 |
|---|---|---|---|---|
| **2024 restart（future 的初始场）** | **464 / 464** | **0** | **0** | **PASS** |
| 2023 全年 h1 | 116 | 1（`time_written`） | 0 | PASS |
| 2023 全年 h0 | 561 | 10 | 0 | PASS |

2024 restart **零豁免、含字符型变量在内全部逐比特一致**。第一版工具报的
"461 个数值变量"少算了 3 个被当作非数值跳过的变量。
...
合计约 110 GB。保存后按测试 2a 的同一套办法校验：**来源校验**（mtime 落在
原生产作业时间窗内）+ **完整性校验**（逐文件 md5）。两者都过才开始重跑。

原 exe 必须留副本而不只是 md5：A3 要重建 exe 才能拿到修复，重建会覆盖
...
- 跑完用 `cmp_nc_strict.py` 逐变量比较新旧 2024 restart
