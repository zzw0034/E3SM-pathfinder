...
### 补充证据（2026-08-17，第三组节点：`pfc057,pfc058`）

在等待 `pfc012,pfc013`（对照用的 NDR 节点，June 那次成功跑用的是
`pfc[012-019]`）从别的作业里释放出来的同时，额外测试了一组此前完全没测过、
当时空闲的 HDR 节点对：`pfc057,pfc058`（`sinfo` 确认两个节点当时都是
`idle`，`IB,HDR,PF`）。

**这次失败模式和之前完全不同**，不是 MPI_Init 阶段的 `"Out of resource"`，
而是启动之后（进程已经过了 MPI_Init）才出现：

```text
flexiblas Failed to allocate memory for hook management. Abort.
...
env.c:1844: env_array_merge: malloc failed
srun: error: pfc058: task 105: Segmentation fault (core dumped)
srun: error: pfc058: tasks 115,134: Segmentation fault (core dumped)
```

同样出现了"部分任务失败、其余任务假死"的模式（`squeue` 一直显示
`RUNNING`，日志文件 10 分钟没有任何新内容）。

关键的两个额外排查点：

1. **在假死作业还占着节点的时候**，用 `srun --jobid=<jobid> --overlap
   -w pfc058 free -h` 现场查看内存占用：`754Gi` 总量，只用了 `18Gi`，
   `279Gi` 空闲。**系统层面内存完全没有耗尽**，但应用层的 `malloc` 却报
   失败——这更像是内存本身有问题（比如某个物理内存条 ECC 故障导致某些
   地址不可用），而不是真的把内存用完了。
2. **`dmesg` 显示多个 `e3sm.exe` 进程几乎同时在 `libc.so.6` 里完全相同的
   代码偏移量上 segfault**（不同 CPU 核心、不同 socket）：这种"多个独立
   进程在同一条指令上崩溃"的模式，是硬件级内存损坏的典型特征，通常不是
   应用程序自己的 bug（应用 bug会在所有节点上都复现，而不是只在特定物理
   机器上）。
3. **错误信息里只出现过 `pfc058`，`pfc057` 一次都没有报错**（`grep -c`
   核实：`pfc057` 命中 0 次，`pfc058` 命中 3 次）。也就是说这一对节点里，
   问题很可能**只出在 `pfc058` 这一台物理机器上**，`pfc057` 大概率是好的，
   只是被同一个 MPI 作业里 `pfc058` 上失败的任务拖累进入假死。

**这修正了 2026-08-17 早些时候的结论**：之前基于 `pfc047,049,051-056`
（全部落在 `IB,HDR,PF` 范围）的 A/B 测试，把失败归因于"HDR 网络代际的
`pfc*` 节点系统性有问题、NDR 代际的没问题"。但这次的证据（同为 HDR 的一对
节点里，一个坏一个大概率是好的，而且失败症状完全不同——一次是 MPI_Init
阶段的资源不足，一次是运行中的内存损坏/segfault）更符合**"个别物理节点有
硬件故障（很可能是内存条），随机分布在 `pfc*` 池子里，因为 HDR 范围
（`pfc031-068`，38 台）本身节点数比 NDR 范围（`pfc003-030`，28 台）多，
统计上更容易先撞到坏节点"**，而不是"HDR 网络代际本身有问题"这种更强的架构
性结论。HDR/NDR 的相关性目前更像是样本量太小造成的巧合，还需要 `pfc012,
pfc013`（NDR 对照）测完之后才能进一步判断。

**How to apply**：不要轻易把个别 `pfc*` 节点的失败推广成"整个 HDR 段
都坏"或"整个 pfc 池子都坏"的结论；每次失败都应该记录具体是哪台物理节点
（`pfc0XX`），failure 症状是哪种（MPI_Init 资源不足 / 运行中 malloc 失败
和 segfault / 其他），方便以后统计是不是集中在少数几台机器上。如果怀疑是
坏内存，作业还占着节点、处于假死状态时可以用
`srun --jobid=<jobid> --overlap -w <node> free -h` 现场探测该节点的真实
内存占用情况，作为"是否真的资源耗尽"还是"资源虚标/硬件故障"的判据。这一层
的根因确认（比如跑 `memtest`、查 BMC/IPMI 硬件错误日志）仍然需要系统管理员
权限，应用层做不到。

### 同事反馈与修正结论（2026-08-17，Dan 提供的外部信息）

同事 Dan 反馈了两条关键背景信息：

1. **pfc 节点用的 InfiniBand 代际和 baseline（blc）节点不一样，pfc 内部各
   节点之间的代际也不统一**——这和我们自己用 `sinfo -N -o '%N %f'` 查到的
   结果吻合（`pfc003-030` 是 `IB,NDR,PF`，`pfc031-068` 是 `IB,HDR,PF`，
   blc 统一是 HDR）。
2. **同事 Nick 之前遇到过类似问题，Solution Center（ORNL 用户支持）已经有
   一个工单/讨论帖在跟这件事；而且"有些软件在两种代际之间混跑没事，但
   E3SM 会出问题"**。

这两条信息把根因的重心从"个别 pfc 物理节点硬件坏了（比如坏内存条）"往前
挪了一步，指向一个更能同时解释我们两种不同失败症状的统一解释：

**IB 混代际（HDR/NDR）子网里，OpenMPI/UCX 的连接协商/资源分配路径可能存在
一个已知问题（bug），会消耗 IB 网卡（HCA）自己管理的一套专用资源（队列对
QP、内存注册表 MR 等，和操作系统层面的 `free -h` 内存是两套完全独立的资源
统计），协商失败或资源用尽时，视卡在哪一步、涉及哪些端点，可能表现成不同
的症状**：

- 卡在建连接阶段 → `MPI_Init` 直接报 `"Out of resource" (-2)`（
  `pfc047,049,051-056` 那次的症状）。
- 协商出的参数或内存注册路径有问题，牵连到应用自己的内存分配器状态 →
  运行中出现 `malloc failed`/segfault（`pfc058` 那次的症状），即使
  `free -h` 显示系统层面内存完全没有耗尽——因为出问题的不是"内存不够"，
  是内存管理/注册路径本身被搅乱了。

这个解释比"pfc058 单台节点内存条坏了"更有说服力，因为它能同时覆盖两种完全
不同的失败症状，也和 Dan、Nick 两个独立的外部观察对得上。"有些软件在两代
之间没问题，但 E3SM 有问题"这句话说明**大概率不是网络硬件彻底损坏**（真
损坏的话所有流量都会受影响），而是 E3SM 所用的这一套 OpenMPI/UCX 通信模式
（集合通信算法、消息大小、连接建立方式）恰好触发了混代际协商里的一个边界
情况，其他通信模式更简单的软件没有触发。

**个别物理节点硬件故障（比如坏内存条）目前降级为次要可能性，但没有被排除**
——`pfc058` 的具体证据（`dmesg` 里多个独立进程在 `libc.so.6` 同一条指令上
segfault）本身仍然更像硬件损坏的特征，两种机制不互斥，也可能是"该节点的
IB 网卡/固件在混代际协商下更容易出问题"这种介于两者之间的情况。

### 目前最好的解决方案

- **短期（现在就要用的生产作业）**：全部使用 `--constraint=BL`（`blc*`
  节点），不使用 `pfc*`，不管是 HDR 还是 NDR 标签的 pfc 节点——因为
  Dan/Nick 反馈的问题指向的是"pfc 池子接入的混代际 IB 子网 + E3SM 的通信
  模式"这个组合，即使某个具体 pfc 节点当次侥幸没触发，也不是可靠保证。这
  是目前唯一经过反复验证、可靠工作的方案，不需要再等更多测试结果就能照做。
- **中期（缩小/确认根因）**：
  - 把我们自己的具体证据（`pfc047,049,051-056` 的 `MPI_Init "Out of
    resource"`、`pfc058` 的 `malloc failed`/segfault、`pfc057` 完全干净
    的对照）发给 Dan，问一下 Solution Center 那个工单里报的错误信息是否
    和我们的一致，工单里是否已经有已知的 workaround（比如强制 OpenMPI/
    UCX 走某种特定 transport、跳过某个有问题的路径的环境变量或 MCA 参数）。
  - `pfc012,pfc013`（NDR 对照）的结果仍然值得等：如果它也失败，说明问题
    不局限于混代际接触点，可能是整个 pfc 子网（不管节点自己是哪代）都受
    共享交换机/子网管理器配置影响；如果它成功，能进一步验证"纯 NDR 环境
    暂时可用"这个更细的判断，但考虑到 Dan 的反馈，这个"细分可用"结论本身
    的可信度已经不如直接全用 blc 高，仅作为诊断信息使用，不建议据此把
    生产作业迁回 pfc。
- **长期**：根因确认（IB 固件/驱动版本、子网管理器配置、UCX/OpenMPI 版本
  兼容性）需要系统管理员或 Solution Center 那边处理，应用层做不到，我们
  这边继续用 `blc*` 规避即可，不需要自己在应用层"修复"这个问题。
